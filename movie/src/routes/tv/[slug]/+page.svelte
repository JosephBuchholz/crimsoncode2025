<script lang="ts">
    ...
    export let data;
    const { show, serverWatched, serverRating } = data;
    ...
    async function updateRating() {
        try {
            const response = await fetch('/api/rate/tv', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
                body: JSON.stringify({ id: show.id, watched: watched, rating: backendRatings[rating], genres: show.genres })
            });
            if (!response.ok) {
...
<div class="flex flex-col m-4 p-4 w-full">
    <div class="flex flex-row w-full">
        <div class="flex flex-col flex-1 m-4">
            <h1 class="text-3xl font-semibold mt-2">{show.name}</h1>
            <p class="text-sm text-gray-600 mt-1">{show.year}</p>

            <p class="text-sm text-gray-600 mt-6">{show.description}</p>
        ...
        <div class="justify-end flex-1">
            <img src={show.imageUrl} alt={show.name} class="rounded-lg" style="margin: auto; max-height: 70vh" />
        </div>
    </div>
</div>
