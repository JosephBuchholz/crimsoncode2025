...
import type { TMDBTVDetailsItem, TMDBTVSearchItem, TMDBTrendingTVResponse } from '$lib/server/tmdb.js';

const prisma = new PrismaClient();

async function fillRemainingRecommendations(recommendations : TMDBTVDetailsItem[], tmdbGenreIds : number[]) {
    ...
    try {
        ...
        while (recommendations.length < 20 && (results == null || results?.length > 0)) {
            ...
            const response : TMDBTrendingTVResponse = await (await fetch("https://api.themoviedb.org/3/trending/tv/week?language=en-US&page=" + page + "&api_key=" + process.env.TMDB_API_KEY)).json();
            ...
            for (let i = 0; i < results.length; i++) {
                ...
                if (recommendations.map((el) => el.id).includes(results[i].id)) {
                    console.log("Rejected " + results[i].name + " (id: " + results[i].id + ") because we already have it in our recommendations.");
                    continue;
                }
                ...
                if (!matchesGenre) {
                    console.log("Rejected " + results[i].name + " (id: " + results[i].id + ") because it does not match our genres.");
                    continue;
                }

                try {
                    const details : TMDBTVDetailsItem = await (await fetch("https://api.themoviedb.org/3/tv/" + results[i].id + "?api_key=" + process.env.TMDB_API_KEY)).json();
...
export async function GET({ request }) {
    ...
    while (true) {
        ...
        try {
            const recommendedShow : TMDBTVDetailsItem = await (await fetch("https://api.themoviedb.org/3/tv/" + recommendedShowId + "?api_key=" + process.env.TMDB_API_KEY)).json();
